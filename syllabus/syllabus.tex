...
\textit{\textbf{Attendance/Absence}: Students needing to reschedule midterms or other major assessments must notify the professor a few days in advance whenever possible.} \\
\textit{\textbf{Late work policy}: Late work is generally not accepted, but exceptions are left to the discretion of the instructor.} \\
\textit{\textbf{Text}: \textit{Introduction to Python Programming}, by Udayan Das, Aubrey Lawson, Chris Mayfield, and Narges Norouzi (OpenStax). The text is available at \url{https://openstax.org/books/introduction-python-programming/pages/1-introduction}.} \\
\textit{\textbf{Software}: Python 3. Students will use the OpenStax code runner and/or a local Python environment. Jupyter notebooks may also be used for numerical, simulation, and data-science exercises.} \\
...
Homework and programming sets & 30\% & Weekly on Fridays \\ \hline
Midterm 1 & 25\% & Wednesday, September 30th \\ \hline
Midterm 2 & 25\% & Wednesday, November 4th \\ \hline
Final programming project & 20\% & Finals period, December 9th--12th \\ \hline
\end{tabular}
...
\textit{\textbf{Grade Settings}: $<60\%$ = F, $60\%$--$<70\%$ = D, $70\%$--$<80\%$ = C, $80\%$--$<90\%$ = B, $90\%$--$100\%$ = A. Pluses and minuses: 0--3\% of each ten-point interval is a minus, 3--6\% is a straight letter grade, and 6--10\% is a plus (for example, 79\% = C+ and 91\% = A-).} \\
...
\begin{itemize}
\item Develop fluency with Python statements, expressions, variables, data types, input/output, and program execution.
\item Use Boolean expressions, conditionals, and loops to implement algorithms and solve computational problems.
\item Decompose programs into reusable functions and modules, and reason about parameters, return values, scope, and control flow.
...
\item Apply numerical programming techniques including floating-point computation, successive approximation, and bisection-style root finding.
\item Design programs using classes, objects, encapsulation, inheritance, and methods.
\item Apply recursion to elementary computational problems and recognize when iterative or recursive solutions are appropriate.
\item Read and write files, work with CSV data, and use exception handling to make programs more robust.
\item Use Python as a problem-solving tool for simulation and introductory data analysis, including basic NumPy, pandas, and visualization.
...
\3 What is computation? What do computer scientists do?
\3 Python programs, statements, and the interpreter
\3 Input/output, variables, strings, numbers, comments, and error messages
\3 \textit{Programming exercise}
\4 Run first Python programs using the OpenStax code runner and/or local Python
\4 Input, computation, formatted output, and reading simple error messages
...
\3 Supplemental representation topics: binary, hexadecimal, bits, bytes, and ASCII/Unicode concepts
...
\3 \textit{Programming exercise, part II}
\4 Implement linear search and compare with Python list operations
...
\4 Emphasis on program tracing, control flow, functions, strings/lists, and basic numerical programming
...
\3 Memoization and repeated subproblems
\3 Merge sort as a recursive divide-and-conquer example
\3 \textit{Programming exercise, part II}
\4 Implement memoized Fibonacci or a similar recursive computation
...
\3 Inheritance basics and ``is-a'' relationships
...
\3 Mixin classes and design tradeoffs
...
\4 Emphasis on strings, lists, dictionaries, classes, recursion, inheritance, and elementary algorithms
...
\3 \textit{Programming exercise, part II}
\4 Random-walk simulation or vacuum-cleaner robot simulation
...
\3 \textit{Programming exercise, part II}
\4 Visualize simulation or tabular data
...
\3 Where to go from here: Computer Science II, data structures and algorithms, scientific computing, and data science
...
\2 Final programming-project presentations/submissions will occur during the College-assigned final examination period.
\2 The exact day and time will follow the Registrar's final-examination schedule for this course section.
